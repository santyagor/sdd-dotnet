...
3. **Remove the default Bootstrap:** this project does **not** use Bootstrap or any CSS framework. The `blazor` template ships with Bootstrap, so remove it right after scaffolding:
   - Delete `wwwroot/lib/bootstrap/` (the bundled Bootstrap library).
   - Delete the template's `wwwroot/bootstrap*` assets if present.
   - Remove any `<link href="...bootstrap...">` references from `App.razor`, `Components/App.razor`, or `index.html`.
   - Remove or replace the default `wwwroot/app.css` / `wwwroot/site.css` template styles that assume Bootstrap.
   - Keep a single canonical `wwwroot/app.css` as the only stylesheet, following the `blazor-app-css-design-system` skill. Do not add Bootstrap, Tailwind, Bulma or any other CSS framework.
4. **Add your first page:** Create a `.razor` file in `Components/Pages/` (server project) or `Pages/` (`.Client` project for WebAssembly components)

## Don'ts

- Don't keep the template's default Bootstrap — this project uses only `wwwroot/app.css`. Remove Bootstrap and never add Bootstrap, Tailwind, Bulma or any other CSS framework.
